...
/**
 * Adapted from react-bits' Ferrofluid background
 * (https://reactbits.dev/backgrounds/ferrofluid) — a WebGL rim-lit fluid
 * blob; only the flowing edges/bands carry color, so most of the canvas
 * stays transparent, which reads as a subtle animated accent rather than a
 * filled panel. Colors are passed in as hex props by the caller, driven by
 * our theme tokens; pauses itself off-screen or when the tab is hidden,
...
const flowVec = (d?: FerrofluidProps["flowDirection"]): [number, number] => {
  switch (d) {
    case "up":
      return [0, 1];
    case "down":
  ...
  useEffect(() => {
    ...
    const loop = (t: number) => {
      ...
      if (mouseDampening > 0) {
        ...
        lastTime = t;
        const factor = Math.min(1, (1 - Math.exp(-dt / Math.max(1e-4, mouseDampening))) || 0);
