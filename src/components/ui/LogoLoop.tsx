...
const isNodeItem = (item: LogoItem): item is Extract<LogoItem, { node: ReactNode }> => "node" in item;
  ...
  const rootClasses = useMemo(
    () =>
      cx(
        ...
        "[--logoloop-fadeColorAuto:var(--color-main-bg)]",
  ...
  const renderLogoItem = useCallback(
    (item: LogoItem, key: Key) => {
      ...
      const content = itemIsNode ? (
        ...
        // Arbitrary runtime `src` values with dynamic sizing — not a fit for next/image.
      ...
      const itemAriaLabel = itemIsNode ? (item.ariaLabel ?? item.title) : (item.alt ?? item.title);
